...
ssh -o StrictHostKeyChecking=no -i $PRIVATE_KEY_FILE $DOCKER_SSH_USER@$DOCKER_SERVER "RATE_VERSION=$RATE_VERSION TRANSACTION_VERSION=$TRANSACTION_VERSION WALLET_VERSION=$WALLET_VERSION UI_VERSION=$UI_VERSION DATA_FAKER_VERSION=$DATA_FAKER_VERSION KAFKA_PORT=$KAFKA_PORT CCLOUD_BOOTSTRAPSERVERS=$CCLOUD_BOOTSTRAPSERVERS CCLOUD_API_KEY=$CCLOUD_API_KEY CCLOUD_API_SECRET=$CCLOUD_API_SECRET docker-compose -f $SERVER_DOCKER_COMPOSE_FILE pull" && \
